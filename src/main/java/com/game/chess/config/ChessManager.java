...
import com.game.chess.view.PromotionSelectionStage;
...
public class ChessManager {
    ...
    private final GamePlatformPane gamePlatformPane = GamePlatformPane.getInstance();
    private final StepRecorder stepRecorder = StepRecorder.getInstance();

    private final Stack<Chess> killedChess = new Stack<>();
    ...
    private ChessManager() {
        chessPane = ChessPane.getInstance();
    }
    ...
    public boolean haveChess(@NotNull Coordinate coord, boolean isBlack) {
        List<Chess> targetedChess = (isBlack) ? black.getOwnedChess() : white.getOwnedChess();
    ...
    private void removeOneChess(@NotNull Coordinate coord, boolean isBlack) {
        if (haveChess(coord)) {
            List<Chess> target = (isBlack) ? black.getOwnedChess() : white.getOwnedChess();
    ...
    public void chessClicked(@NotNull Chess chess, @NotNull String newType) {
        List<Chess> target = (chess.isBlack()) ? black.getOwnedChess() : white.getOwnedChess();
        Chess newChess = null;
        int i;
    ...
    public List<Chess> getChess(boolean isBlack) {
